 ...
 *  
 */
void ControlRobot::recorrerCamino() {
    
}

bool ControlRobot::nodoAlcanzado(){
...
void ControlRobot::imprimirInfo(void) {
    ...
    switch (estado_actual) {
        ...
        case SIGUE: sprintf(estado, "SIGUE");
            break;
        case BUSCA_D: sprintf(estado, "BUSCA_D");
            break;
        case BUSCA_I: sprintf(estado, "BUSCA_I");
            break;
    }
...
int ControlRobot::avanzar() {
    sensores.front_left = robot->updateSensor(iRobotSensors::CLIFFFRONTLEFTSIGNAL);
    sensores.front_right = robot->updateSensor(iRobotSensors::CLIFFFRONTRIGHTSIGNAL);
    sensores.bumper = robot->updateSensor(iRobotSensors::BUMPERS_AND_WHEELDROPS);

    sensores.fl = DUCT_TAPE_FL(sensores.front_left);
    sensores.fr = DUCT_TAPE_FR(sensores.front_right);
    sensores.bp = sensores.bumper > 0 && sensores.bumper < 4;

    robot->driveDirect(VEL_AVANCE, VEL_AVANCE);
    bool choque = false;
    
    while (!(sensores.fl && sensores.fr) && !choque) {
        choque = false;
        if (sensores.fl && !sensores.fr) {
            robot->driveDirect(VEL_GIRO, -VEL_GIRO);
